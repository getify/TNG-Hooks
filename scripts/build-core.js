...
var fs = require("fs"),
	...
	CORE_SRC = path.join(SRC_DIR, "tng-hooks.src.js"),
	CORE_DIST = path.join(DIST_DIR, "tng-hooks.js"),
	result = "";
...
try {
	...
	// result = ugly.minify(path.join(SRC_DIR,"tng-hooks.src.js"),{
	// 	mangle: {
	// 		keep_fnames: true
	// 	},
	// 	compress: {
	...
	packageJSON = JSON.parse(
		fs.readFileSync(path.join(ROOT_DIR, "package.json"), {
			encoding: "utf8"
		})
	);
	...
	copyrightHeader = fs
		.readFileSync(path.join(SRC_DIR, "copyright-header.txt"), {
			encoding: "utf8"
		})
		.replace(/`/g, "");
	copyrightHeader = Function(
		"version",
		"year",
